...
class BaseTestCarQueue(unittest.TestCase, HeapAssertions):
    ...
    def comparisons_test(self, test_file_name):
        ...
        message = f"Your code used {int(queue.num_comps)} comparisons "
        message += f"but was expected to use greater or equal to {int(lower_bound)}."
        self.assertGreaterEqual(queue.num_comps, lower_bound, msg=message)
        message = f"Your code used {int(queue.num_comps)} comparisons "
        message += f"but was expected to use less than or equal to {int(upper_bound)}."
        self.assertLessEqual(queue.num_comps, upper_bound, msg=message)
        return True
    ...
    def internal_comparisons_test(self, test_file_name):
        ...
        import_data, instructions = read_heap_test_data(test_file_name)
        queue = self.run_test_file_instructions(import_data, instructions)
        message = f"Your code reported using {int(queue.num_comps)} comparisons "
        message += f"but it actually used {int(get_real_count(PRIORITY_COMPS))}. "
...
def all_tests_suite():
    ...
    # Task 2 - Enqueue
    #suite.addTest(test_loader(TestTaskTwoTiny))
    #suite.addTest(test_loader(TestTaskTwoSmall))
    #suite.addTest(test_loader(TestTaskTwoMedium))
    #suite.addTest(test_loader(TestTaskTwoLarge))
    #suite.addTest(test_loader(TestTaskTwoHuge))
    suite.addTest(test_loader(TestTaskTwoAll))

    # uncomment the following lines when ready to test further tasks...

    # Task 3 - Dequeue
    #suite.addTest(test_loader(TestTaskThreeTiny))
    #suite.addTest(test_loader(TestTaskThreeSmall))
    #suite.addTest(test_loader(TestTaskThreeMedium))
    #suite.addTest(test_loader(TestTaskThreeLarge))
    #suite.addTest(test_loader(TestTaskThreeHuge))
    # suite.addTest(test_loader(TestTaskThreeAll))

    # OPTIONAL Task 1 - update with the EditableCarHeapqueue
    # suite.addTest(test_loader(TestTaskFourAll))

    # OPTIONAL Task 2 - remove with the EditableCarHeapqueue
    # suite.addTest(test_loader(TestTaskFiveAll))
